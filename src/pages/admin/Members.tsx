...
export default function Members() {
  ...
  const roleOptions: Role[] = useMemo(
    () => ["unassigned", "employee", "hr", "accounts"], // admin stays with first user
    []
  );
  ...
  const bootstrap = async () => {
    ...
    try {
      ...
      const role = (myMembership.role ?? "unassigned") as Role;
      ...
      // NOTE: this expects a `profiles` table with (id, full_name, email)
  ...
  const updateRole = async (membershipId: string, newRole: Role) => {
    ...
    try {
      ...
      toast({
        title: "Role updated",
        description: `Member role set to ${newRole}`,
      });
    } catch (e: any) {
      console.error(e);
      toast({
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              New users will appear as <b>unassigned</b>. Set them to employee, hr, or accounts.
            </CardDescription>
          </CardHeader>
            ...
            {members.length === 0 ? (
              ...
              members.map((m) => (
                ...
                >
                  ...
                  {/* Keep admin as admin (don’t downgrade) */}
                  {m.role === "admin" ? (
